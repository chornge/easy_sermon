...
# fmt: off
# Map all 66 books to their chapters and verses
BIBLE_STRUCTURE = {
    ...
    "3 John": [14],
    "Jude": [25],
    "Revelations": [20,29,22,11,14,17,17,13,21,11,19,17,18,20,8,21,18,24,21,15,21,21],
}

# fmt: on
BOOKS = [book.lower() for book in BIBLE_STRUCTURE]
...
def extract_bible_reference(text):
    ...
    for match in REFERENCE_PATTERN.findall(text):
        ...
        if chapter is None or verse_start is None:
            print(
                f"⚠️ Skipping invalid reference: {book} {chapter_raw}:{verse_start_raw}"
            )
            continue

        # ⛔️ Validate book and chapter against BIBLE_STRUCTURE
        if book not in BIBLE_STRUCTURE:
            print(f"⚠️ Unknown book: {book}")
            continue

        try:
            chapter_num = int(chapter)
            verse_num = int(verse_start)
            if chapter_num < 1 or chapter_num > len(BIBLE_STRUCTURE[book]):
                print(f"⚠️ Invalid chapter: {book} {chapter_num}")
                continue
            if verse_num < 1 or verse_num > BIBLE_STRUCTURE[book][chapter_num - 1]:
                print(f"⚠️ Invalid verse: {book} {chapter_num}:{verse_num}")
                continue
        except ValueError:
            print(
                f"⚠️ Could not convert chapter or verse to int: {chapter}, {verse_start}"
            )
            continue
        ...
        if verse_end:
            try:
                ...
                if verse_end_num > BIBLE_STRUCTURE[book][chapter_num - 1]:
                    print(f"⚠️ Invalid verse range end: {book} {chapter}:{verse_end}")
                    continue
